...
| 1.0.5 | — | 애니메이션 속도 조정 (420ms → 560ms, 원본·현재 중간값) |
| 1.1.0 | — | 색상 팔레트 개선 — pink·teal·indigo 색 구분 강화 |
| 1.1.1 | — | 병 완성 시 흔들기 애니메이션 및 효과음 추가 |
| 1.1.2 | — | Service Worker 추가 — iOS 홈 화면 앱 자동 업데이트 |

### 버전 업 절차
1. `index.html` 첫 줄 `<!-- version: X.Y.Z -->` 숫자 수정
2. `sw.js` 첫 줄 `// version: X.Y.Z` 및 `const CACHE = 'water-sort-X.Y.Z'` 동일하게 수정 ← **필수** (미수정 시 iOS 홈 화면 앱 캐시 갱신 안 됨)
3. 커밋 메시지에 버전 포함: `[v1.0.3] 수정 내용`
4. push
